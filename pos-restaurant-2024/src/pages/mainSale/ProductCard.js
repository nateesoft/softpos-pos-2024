...
const ProductCard = memo(({ id, product, openModal, initLoadMenu, initLoadOrder }) => {
    ...
    const addOrder = async (qty = 1) => {
        ...
        axios.post(`/api/balance`, payload)
            ...
            .catch((error2) => {
                console.log('(error2)=> create in balance table', error2)
            })
    }
    return (
        <Badge id={id} badgeContent={product.qty} color="primary" sx={{ "& .MuiBadge-badge": { fontSize: 18, height: 25, minWidth: 35, top: 15, right: 18, borderRadius: 1, color: "snow", fontWeight: "bold" } }}>
            <div style={{ border: "1px solid #eee", padding: "5px", borderRadius: "8px 8px 0px 0px", boxShadow: "2px 1px #eee" }}>
                <Box textAlign="center">
                    <img src={product.image_url} alt="" width={160} style={{ borderRadius: "8px 8px 0px 0px" }} onClick={openModal} /><br />
                </Box>
                ...
                <Grid container justifyContent="space-between">
                    <Typography sx={{ marginLeft: "2px", fontWeight: product.show_list_menu === 'Y' ? "bold" : "none", color: product.show_list_menu === 'Y' ? "green" : "black" }}>ราคา {product.menu_price}</Typography>
                    <Typography sx={{ marginRight: '2px' }}>{product.tab_group}</Typography>
                </Grid>
                <Grid container>
                    {product.show_list_menu === 'Y' && <Button fullWidth variant="contained" sx={{ backgroundColor: "salmon", color: "black", marginTop: "5px" }} startIcon={<SetMealIcon />}>Menu Set</Button>}
                    {product.show_list_menu !== 'Y' && <Button color="success" fullWidth variant="contained" sx={{ marginTop: "5px" }} onClick={() => addOrder(1)} startIcon={<AddCircleOutline />}>Order</Button>}
                </Grid>
            </div>
        </Badge>
    )
