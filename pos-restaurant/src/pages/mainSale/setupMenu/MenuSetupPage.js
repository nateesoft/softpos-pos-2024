            ...
            >
              <FormControl sx={{ width: "150px" }}>
                ...
                >
                  {menuTabs &&
                    menuTabs.map((item) => (
                      <MenuItem key={item.tab_key} value={item.tab_key}>
                        {item.tab_key}
                      </MenuItem>
                    ))}
                </Select>
                ...
                >
                  {productList &&
                    productList.map((item) => (
                      <MenuItem value={item.PCode}>{item.PDesc}</MenuItem>
                    ))}
                </Select>
              </FormControl>
              <TextField
        ...
        <Grid2 container spacing={1} padding={1}>
          <Grid2 size={12}>
            <FormControlLabel
              ...
              label="ไม่คิดราคา (Free Price)"
            />
            <FormControlLabel
              control={
                <Checkbox
                  value={manualPrice}
                  onChange={() => setManualPrice(!manualPrice)}
                />
              }
              label="กำหนดราคาเอง (Manual Price)"
            />
            <FormControlLabel
              control={
                <Checkbox
                  value={showListMenu}
                  onChange={() => setShowListMenu(!showListMenu)}
                />
              }
              label="แสดงสินค้าใน SET ให้เลือก"
            />
          </Grid2>
        </Grid2>
        {showListMenu && (
          ...
          >
            ...
            <Grid2 container spacing={1} padding={1}>
              <Grid2 size={6}>
                <Grid2 container spacing={1}>
                  <TextField
                    label="ที่อยู่รูปภาพ เมนูย่อย"
                    value={menuUrl2}
                    onChange={(e) => setMenuUrl2(e.target.value)}
                    sx={{ minWidth: "350px" }}
                  />
              ...
              <Grid2 size={6}>
                <FormControlLabel
                  ...
                  label="เลือกทันที เมื่อเลือกเมนูหลัก"
                />
                <FormControlLabel
                  control={
                    <Checkbox
                      value={canChange}
                      onChange={() => setCanChange(!canChange)}
                    />
                  }
                  label="เปลี่ยนเมนูได้"
                />
                <FormControlLabel
                  control={
                    <Checkbox
                      value={freePrice2}
                      onChange={() => setFreePrice2(!canChange)}
                    />
                  }
                  label="ไม่คิดราคาเพิ่ม"
                />
                <Button
                  variant="contained"
                  color="success"
                  startIcon={<AddItem />}
                  onClick={addMenuItemList}
                >
                  เพิ่มรายการ
                </Button>
              </Grid2>
